...
class CarlaQADataset(Dataset):
    ...
    def _get_waypoints(self, sources):
        ...
        # update prompts
        conversations[0]['value'] = conversations[0]['value'].replace('[target_value]', '({:.1f}, {:.1f})'.format(target_waypoint[0], target_waypoint[1]))
        conversations[1]['value'] = conversations[1]['value'].replace(
                                    '[local_waypoints]','({:.1f}, {:.1f}), ({:.1f}, {:.1f}), ({:.1f}, {:.1f}), ({:.1f}, {:.1f}), ({:.1f}, {:.1f})'
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if 'local_future_waypoints' in sources[0].keys():
                ...
                # local_future_waypoints = torch.Tensor((sources[0]['local_future_waypoints']))
        ...
        # update data dict for agent learning
        data_dict.update(
            {
