...
class MobileLlamaForCausalLM(LlamaForCausalLM, MobileVLMMetaForCausalLM):
    ...
    def __init__(self, config):
        ...
        self.agent_head = nn.Sequential(
            torch.nn.Linear(config.hidden_size, 1024),
            torch.nn.ReLU(),
    ...
    def forward(
        ...
        **kwargs,
        ...
        # check here
        pooled_feature = self.adp_pooling(hidden_states.permute(0, 2, 1).detach())   # [B, 10, 1]
        agent_outputs = self.agent_head(pooled_feature.permute(0, 2, 1)).reshape(-1, 5, 2)
        ...
        if labels is not None:
            ...
            loss = loss_fct(shift_logits, shift_labels) * self.llm_loss_weight
            
            if 'local_future_waypoints' in kwargs.keys():
                local_wp_gt = kwargs['local_future_waypoints']
                valid_wp_mask = kwargs['valid_future_waypoints']
                agent_loss = nn.functional.l1_loss(agent_outputs, local_wp_gt, reduction='none') * valid_wp_mask
                agent_loss = agent_loss.mean()
